...
class PublicationController extends Controller
{
    ...
    {
        $publications = Publication::where('user_id',userAuthId())->where('is_active', true)->orderBy('id','asc')->get();
    ...
    {
        $request->validate([
            'title' => ['required', 'string', Rule::unique('publications')->where(fn ($query) => $query->where('user_id', userAuthId()))],
            'date' => ['nullable', 'date'],
            'url' => [
                'nullable',
                'url',
                'regex:/^((https?|ftp):\/\/)?([a-zA-Z0-9.-]+)\.([a-zA-Z]{2,})(\/\S*)?$/',
                Rule::unique('publications')->where(fn ($query) => $query->where('user_id', userAuthId()))
            ],
    ...
    {
        $publication = Publication::where('slug', $slug)->where('user_id', userAuthId())->firstOrFail();
        $request->validate([
            'title' => ['required', 'string', Rule::unique('publications')->where(fn ($query) => $query->where('user_id', userAuthId()))->ignore($publication->id)],
            'date' => ['nullable', 'date'],
            'url' => [
                'nullable',
                'url',
                'regex:/^((https?|ftp):\/\/)?([a-zA-Z0-9.-]+)\.([a-zA-Z]{2,})(\/\S*)?$/',
                Rule::unique('publications')->where(fn ($query) => $query->where('user_id', userAuthId()))->ignore($publication->id)
            ],
            'desc' => ['required', 'string'],
        ], [
            'url.regex' => 'The URL format is invalid. Please enter a valid web address.',
        ...
        // $publication = publication::where('user_id', userAuthId())->where('slug', $slug)->first();
