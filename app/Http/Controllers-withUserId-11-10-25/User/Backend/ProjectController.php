...
class ProjectController extends Controller
{
    ...
    {
        $projects = Project::where('user_id',userAuthId())->where('is_active', true)->orderBy('id','asc')->get();
    ...
    {
        $request->validate([
            'title' => ['required', 'string', Rule::unique('projects')->where(fn ($query) => $query->where('user_id', userAuthId()))],
            'date' => ['nullable', 'date'],
            'url' => [
                'nullable',
                'url',
                'regex:/^((https?|ftp):\/\/)?([a-zA-Z0-9.-]+)\.([a-zA-Z]{2,})(\/\S*)?$/',
                Rule::unique('projects')->where(fn ($query) => $query->where('user_id', userAuthId()))
            ],
    ...
    {
        $project = Project::where('slug', $slug)->where('user_id', userAuthId())->firstOrFail();
        $request->validate([
            'title' => ['required', 'string', Rule::unique('projects')->where(fn ($query) => $query->where('user_id', userAuthId()))->ignore($project->id)],
            'date' => ['nullable', 'date'],
            'url' => [
                'nullable',
                'url',
                'regex:/^((https?|ftp):\/\/)?([a-zA-Z0-9.-]+)\.([a-zA-Z]{2,})(\/\S*)?$/',
                Rule::unique('projects')->where(fn ($query) => $query->where('user_id', userAuthId()))->ignore($project->id)
            ],
            'image' => ['required', 'image', 'mimes:jpeg,png,jpg,gif', 'max:2048', 'dimensions:ratio=1/1'],
            'desc' => ['required', 'string'],
        ], [
        ...
        // $project = project::where('user_id', userAuthId())->where('slug', $slug)->first();
        $project->update($data);
