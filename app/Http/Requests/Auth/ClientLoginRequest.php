...
class ClientLoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        if (! Auth::guard('user')->attempt([$fieldType => $login, 'password' => $this->input('password')], $this->boolean('remember'))) {
            RateLimiter::hit($this->throttleKey());

            throw ValidationException::withMessages([
                'login' => $fieldType === 'email'
                    ? 'The provided email and password do not match our records.'
                    : 'The provided phone number and password do not match our records.',
            ]);
        }
    ...
    {
        return Str::transliterate(Str::lower($this->string('login')).'|'.$this->ip().'|'.$this->header('User-Agent'));
    }
