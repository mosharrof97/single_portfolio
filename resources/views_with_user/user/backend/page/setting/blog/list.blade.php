...
<div class="container-xl">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            ...
            <a href="{{ route('user.blog.create') }}" class="btn btn-primary">Create blog post</a>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('user.blog.edit',$data->slug) }}" class="btn btn-primary m-1">Edit</a>

                                <form action="{{ route('user.blog.delete',$data->slug) }}" method="post">
                                    @csrf
                                    @method('delete')
...
<script>
    $(document).ready(function() {
        $('.status').change(function() {
            ...
            $.ajax({
                type: 'PUT'
                , url: "{{ route('user.blog.update.status', ':slug') }}".replace(':slug', slug)
                , data: {
                    _token: '{{ csrf_token() }}'
                    , slug: slug
                    , status: status
