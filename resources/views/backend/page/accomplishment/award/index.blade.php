        ...
        <div class="card-body">
            <div id="commonForm_aca">
                <script>
                    $(document).ready(function() {
                        ...
                        function initializeForm(Id) {
                            $(`#award_view_${Id}`).show();
                            $(`#edit-tools_${Id}`).show();
                            $(`#award_edit_${Id}`).hide().find('input, select, textarea').prop('disabled', true);
                        }

                        // let oldurl = @json(old('url'));
                        // if(oldurl){
                        //     const Id = $('.edit_btn').data('id');
                        //     $(`#award_view_${Id}, #edit-tools_${Id}`).hide();
                        //     $(`#award_edit_${Id}`).show().find('input, select, textarea').prop('disabled', false);
                        // }
                ...
                @foreach ($awards as $key => $data)
                    <div class="modal fade" id="deleteaward_{{ $data->id }}" data-bs-backdrop="static"
                        data-bs-keyboard="false" tabindex="-1" aria-labelledby="deleteawardLabel_{{ $data->id }}"
                        aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-body p-5">
                                    <h1 class="modal-title fs-4" id="deleteawardLabel_{{ $data->id }}"> <i
                                    ...
                                    <div class="text-end">
                                        ...
                                        <a href="{{ route('award.delete', $data->slug) }}" class="btn btn-danger">YES,
                                            DELETE</a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="award_view" id="award_view_{{ $data->id }}">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div class="edit-tools" id="edit-tools_{{ $data->id }}" style="">
                                            <button class="btn btn-primary edit_btn" id="upd_edit_btn_{{ $data->id }}"
                                                data-id="{{ $data->id }}" aria-label="Edit award"><i
                                                    class="icon-pencil-o"></i>Edit</button>
                                            <button class="btn btn-danger" aria-label="Delete award" data-bs-toggle="modal"
                                                data-bs-target="#deleteaward_{{ $data->id }}">Delete</button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="award_edit" id="award_edit_{{ $data->id }}" style="display: none">
                        <form action="{{ route('award.update', $data->slug) }}" class="" id=""
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="text-end">
                                        <input type="submit" class="btn btn-primary" title="Save new award information"
                                            value="Save">
                ...
                <div class="mt-5" id="div_aca" style="display: none">
                    <div class="">
                        <div class="sub-header">
                            ...
                            <h4>Award <b class="fst-italic" style="color:#525252;">(Max 5)</b></h4>
                            <div class="edit-tools" style="display: none;"><button class="btn edit-btn"
                                    aria-label="Edit award"><i class="icon-pencil-o"></i>&nbsp;Edit</button><button
                                    class="btn delete-btn" aria-label="Delete award">&nbsp;Delete</button></div>
                        </div>
                        <form action="{{ route('award') }}" class="" id="" method="POST"
